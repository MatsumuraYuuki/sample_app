...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with valid email" do
    ...
    # ActionMailer が送信したメールの数が 1 通であることを確認
  ...
  #パスワードリセット機能において、非アクティブなユーザーがパスワードリセットを試みた際に、適切にリダイレクトされるかどうかを検証しています
  test "reset with inactive user" do
    @reset_user.toggle!(:activated)  #toggle! メソッドは、指定された属性の値を反転させます
  ...
  #正しいメールアドレスと正しいリセットトークンを使用してパスワードリセットページにアクセスできることを検証
  test "reset with right email and right token" do
    get edit_password_reset_path(@reset_user.reset_token,
                                 email: @reset_user.email)
    assert_template 'password_resets/edit'
    # HTMLのフォームに隠しフィールド（<input>タグ）が存在し、その name属性がemailであり、type属性が hidden で、value属性が@reset_user.emailであることを確認しています。これは、正しいメールアドレスがフォームに含まれていることをチェックするためです。
...
class PasswordUpdateTest < PasswordResetForm
  ...
  test "update with valid password and confirmation" do
    ...
    # 12.3.３演習 データベースから再読み込みされたユーザーオブジェクトのreset_digest属性がnilであることを確認
    assert_nil @user.reload.reset_digest
  end
end
